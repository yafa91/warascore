...
export function AppContent() {
 ...
 const leaguesList = [
  ...
  { id: 62, name: "Ligue 2", logo: "https://media.api-sports.io/football/leagues/62.png" },
...
useEffect(() => {
 ...
 const finished = europeanMatches.filter((match: Match) => {
  const statusTerminated = ["FT", "AET", "PEN"].includes(match.fixture.status.short);
...
const isPriorityMatch = (match: Match) => {
  ...
  if (round.includes("semi") || round.includes("demi") || round.includes("final")) {
    return true;
  }
  ...
  if (
     (home.includes("paris") && away.includes("marseille")) ||
     (home.includes("marseille") && away.includes("paris")) ||

     (home.includes("real") && away.includes("barcelona")) ||
     (home.includes("barcelona") && away.includes("real"))
  ) {
    return true;
  }
...
<View style={{ flexDirection: "row", marginTop: 20 }}>
  {selectedDate >= getCurrentDate() && (
    ...
    >
      <Text style={{ color: "#fff", fontWeight: "bold" }}>Prochains Matchs</Text>
    </TouchableOpacity>
  )}
  ...
  {finishedMatches.length > 0 && (
    ...
    >
      <Text style={{ color: "#fff", fontWeight: "bold" }}>Matchs Terminés</Text>
    </TouchableOpacity>
  )}
  ...
  >
    <Text style={{ color: "#fff", fontWeight: "bold" }}>Classement</Text>
  </TouchableOpacity>
</View>
...
{selectedTab === "classement" ? (
  ...
  renderItem={({ item }) => (
    <TouchableOpacity
      ...
      style={{
        marginRight: 15,
        backgroundColor: selectedLeagueId === item.id ? "#E40000" : "#222",
        borderRadius: 10,
        paddingVertical: 3,
        paddingHorizontal: 8,
        flexDirection: "row",
    ...
    >
      <Image source={{ uri: item.logo }} style={{ width: 20, height: 20, marginRight: 6 }} />
...
/>
    ...
    {loadingStandings ? (
      <Text style={{ color: "#fff", textAlign: "center", marginTop: 20 }}>Chargement du classement...</Text>
    ) : standings.length === 0 ? (
      <Text style={{ color: "#A8A5A5", textAlign: "center", marginTop: 20, fontStyle: "italic" }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            <Text style={{ color: "#fff", width: 30 }}>{item.rank}</Text>
            <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
              <Image
                source={{ uri: item.team.logo }}
                style={{ width: 20, height: 20, marginRight: 8}}
              />
              <Text style={{ color: "#fff" }}>{item.team.name}</Text>
            </View>
            <Text style={{ color: "#fff", width: 30, textAlign: "right" }}>{item.points}</Text>
          </View>
        )}
      />
    )}
  ...
  <FlatList
    ...
    data={selectedTab === "upcoming" ? upcomingMatches : finishedMatches}
    keyExtractor={(item) => item.fixture.id.toString()}
    ...
    ListEmptyComponent={() => (
      <View style={{ width: "100%", alignItems: "center", padding: 10 }}>
        <Text style={{ color: "#A8A5A5", fontStyle: "italic" }}>
          Aucun match {selectedTab === "upcoming" ? "à venir" : "terminé"}
        </Text>
      </View>
    )}
  />
          ...
          <View style={styles.scoreListTeams}>
            <View style={styles.scoreListTeamRow}>
              ...
              <Text numberOfLines={1} style={styles.teamName}>
               ...
               {!finishedMatches(item)&& <Text style={styles.teamName}>{item.goals.home}</Text>}
              </View>
            </View>
            <View style={styles.scoreListTeamRow}>
              <Image
              ...
              <View style={{flexDirection:"row", justifyContent:"space-between", flex:1}}>
                <Text numberOfLines={1} style={styles.teamName}>
                 ...
                 {!finishedMatches(item)&& <Text style={styles.teamName}>{item.goals.away}</Text>}
              </View>
            </View>
          </View>
